...
def pull_up_header(fw, count):
    fw.write("Library IEEE;\n")
    fw.write("USE IEEE.STD_LOGIC_1164.ALL;\n")
    fw.write("USE IEEE.STD_LOGIC_UNSIGNED.ALL;\n")
    fw.write('\n')

    fw.write("entity pull_up is\n")
    fw.write("\tport(\n")
    fw.write("\t\tclk: in std_logic;\n")
    fw.write("\t\treset: in std_logic;\n")
    for i in range (count-1):
        fw.write("\t\tisMatching_" + str(i) + ": in std_logic;\n")
    fw.write("\t\tisMatching_" + str(count-1) + ": in std_logic;\n")
    for i in range (count-1):
        fw.write("\t\tpull_up_" + str(i) + ": out std_logic;\n")
    fw.write("\t\tpull_up_" + str(count-1) + " : out std_logic\n")
    fw.write("\t);\n")
    fw.write("end pull_up;\n\n")
...
def pull_up_signals(fw, count):
    ...
    for i in range(count):
        fw.write("signal release_" + str(i) + ": std_logic;\n")

def pull_up_port(fw, count):
    for i in range(count):
         fw.write("\tpull_" + str(i) + ": pull_up_module")
         fw.write(" port map(clk => clk, reset => reset, isMatching => isMatching_" + str(i) + " out_isMatching => hold_" + str(i) + ");\n")
    
    for i in range(count):
         fw.write("\tFF_out_" + str(i) + ": FF")
         fw.write(" port map(clk => clk, reset => reset, input => release_" + str(i) + " output => pull_up_" + str(i) + ");\n")
    for i in range(count):
         fw.write("\trelease_" + str(i) + " <= hold_" + str(i) + ";\n")



def pull_up_arch(fw, hex_list):
    fw.write("architecture behavioral of pull_up is\n\n")
...
def main():
    ...
    if(len(sys.argv)!=2):
        ...
        print "example: python seq_match.py inputFile.txt outputFile"
        sys.exit()
    ...
    fw = open('pull_up.vhd', 'w')
    pull_up_header(fw, len(hex_list))
    pull_up_arch(fw, hex_list);
